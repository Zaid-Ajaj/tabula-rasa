...
let title state dispatch =
    let publishBtnContent =
        if state.SavingChanges then i [ ClassName "fa fa-circle-o-notch fa-spin" ] []
    ...
    h1 [] [ str "Edit Post"
            ...
            button [ ClassName "btn btn-success"
                     Style [ MarginLeft 15 ]
                     OnClick(fun _ -> dispatch SaveChanges) ] [ publishBtnContent ] ]
...
let editor (article : BlogPostItem) dispatch =
    div [ Style [ Margin 10 ] ] [ div [] [ titleAndSlug article dispatch
                                           br []
...
let preview (article : BlogPostItem) =
    div [ ClassName "card"
          Style [ Padding 20 ] ] [ Marked.marked [ Marked.Content article.Content
                                                   Marked.Options [ Marked.Sanitize false ] ] ]

let body isPreview (article : BlogPostItem) dispatch =
    if isPreview then preview article
