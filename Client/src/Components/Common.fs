...
let spinner = div [ ClassName "cssload-container" ] [ div [ ClassName "cssload-whirlpool" ] [] ]

let icon isLoading name =
    let className =
        if isLoading then "fa fa-circle-o-notch fa-spin fa-fw"
...
module Cmd =
    ...
    let fromAsync (asyncParams : AsyncValParams<'a, 'b>) : Elmish.Cmd<'b> =
        Elmish.Cmd.ofAsync (fun () -> asyncParams.Value) () asyncParams.Success asyncParams.Error
