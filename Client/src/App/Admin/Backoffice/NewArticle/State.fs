...
let update authToken msg (state : NewArticleState) =
    ...
    | TogglePreview -> 
        ...
        nextState, Cmd.none
    | Publish -> 
        if state.IsPublishing then state, warning "Publishing in progress..."
        elif state.Title = "" then state, warning "Title of your blog post cannot be empty"
        elif state.Slug = "" then state, warning "The slug cannot be empty"
        else 
            ...
            let publishCmd =
                Cmd.fromAsync { Value = Server.api.publishNewPost request
    ...
    | SaveAsDraft -> 
        if state.IsPublishing then state, warning "Publishing in progress..."
            ...
            nextState, 
            Cmd.ofAsync Server.api.savePostAsDraft request successHandler 
    ...
    | AddTag -> 
        ...
        if String.length tag = 0 then state, Toastr.info (Toastr.message "Tag empty")
        else 
