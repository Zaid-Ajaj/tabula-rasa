...
let guestAdmin : AdminInfo =
    let info : CreateAdminReq =
        { Name = "Guest Guest"
          ...
          ProfileImageUrl =
              "https://user-images.githubusercontent.com/13316248/31862023-6bb4bb10-b737-11e7-9de3-58ca0b1644c3.jpg" }
    create info
...
let updatePassword (logger : ILogger) (db : LiteDatabase) =
    Security.authorizeAdmin <| fun updatePassInfo user -> 
        ...
        | Some admin -> 
            ...
            if not passwordDidMatch then Error "The input for your current password is incorrect"
            else 
                let salt = createRandomKey()
                ...
                let modifiedAdmin =
                    { admin with PasswordSalt = base64 salt
                                 PasswordHash = base64 passwordHash }
...
let validateBlogInfo (blogInfo : BlogInfo) =
    if String.IsNullOrWhiteSpace blogInfo.BlogTitle then Some "Blog Title cannot be empty"
    elif String.IsNullOrWhiteSpace blogInfo.Name then Some "The name of the blog cannot be empty"
    else None
        ...
        | Some foundAdmin -> 
            ...
            | None -> 
                let modifiedAdminInfo =
                    { foundAdmin with Name = blogInfo.Name
                                      Bio = blogInfo.Bio
                                      About = blogInfo.About
                                      ProfileImageUrl = blogInfo.ProfileImageUrl
                                      BlogTitle = blogInfo.BlogTitle }
                if admins.Update(modifiedAdminInfo) then Ok(SuccessMsg "Updated succesfully")
