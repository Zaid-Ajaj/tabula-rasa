...
let authorize (claims : string list) (f : 'u -> UserInfo -> 't) : SecureRequest<'u> -> SecureResponse<'t> =
    fun request -> 
        ...
        | Some user -> 
            let userHasAllClaims = claims |> List.forall (fun claim -> Array.contains claim user.Claims)
            if not userHasAllClaims then async { return Result.Error AuthError.UserUnauthorized }
            else 
                async { 
...
let authorizeAsync (claims : string list) (f : 'u -> UserInfo -> Async<'t>) : SecureRequest<'u> -> SecureResponse<'t> =
    fun request -> 
        ...
        | Some user -> 
            let userHasAllClaims = claims |> List.forall (fun claim -> Array.contains claim user.Claims)
            if not userHasAllClaims then async { return Result.Error AuthError.UserUnauthorized }
            else async { let! output = f request.Body user
                         return Result.Ok output }
